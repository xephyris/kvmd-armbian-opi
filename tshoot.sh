...
if [ $ARCHLINUX -eq 1 ]; then
  ...
  PLATFORM=$( pacman -Q | grep kvmd-platform | cut -d'-' -f3,4,5 )
else
...
if [ $HDMIUSB -ge 1 ]; then 
  ...
  if [[ $( echo $USBHDMI | grep -c $SEARCH ) -gt 0 ]]; then
    ...
    echo "$USBHDMI"
    echo
    echo "*NOTE:  USB HDMI max supported resolution is 4k 30hz but will be downsampled to 1080p"
  else
...
printf "\n--- KNOW THE LIMITS AND MAKE SURE TARGETS STAY WITHIN THOSE LIMITS ---\n"
